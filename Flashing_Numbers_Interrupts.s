...
/* Enable IRQ interrupts in the ARM processor */
    ...
    LDR      R6, =0xFF200000  // LEDR base address

    // PART A CODE   
    LOOP:   
    LDR     R1, =0xFF200040     // load SW into R1 
    LDR     R1, [R1] 
    BL      SEG7_CODE           // convert R1 into seg 7 and store in R0 
...
/* Configure the Interval Timer to create interrupts at 0.5 second intervals */
// Code from ARM A9 Documentation (section 3.6)
CONFIG_TIMER:
    LDR	      R0, =MPCORE_PRIV_TIMER
    LDR       R1, =50000000         // 0.5s constant 
    STR       R1, [R0]              // load value into timer 
    
    MOV       R1, #0b111            // enable timer 
    STR       R1, [R0, #0x8]
    BX        LR    
...
TIMER_ISR:
    ...
    CMP       R1, #1       // load previous value, if 1 make it 0, vice versa 
    MOVEQ     R1, #0
    MOVNE     R1, #1 
    LDR       R2, =RUN 
    STR       R1, [R2] 

    LDR       R2, =MPCORE_PRIV_TIMER //clear interupt
    MOV       R1, #0b1 
    STR       R1, [R2, #0xC]
...
SEG7_CODE:  LDR     R2, =BIT_CODES  
            ADD     R2, R1         //R1 is number to be loaded
            LDRB    R0, [R2]       //R0 is returned as seg 7 code
            MOV     PC, LR   
...
HEX_DISPLAY:
	   //checks count is 0 or 1
	   LDR R0, =COUNT
	   LDR R0, [R0]
	   CMP R0,#0x0
	   
	   // if count is 1 store value normally
	   STRNE R4, [R6, #0x20]
	   STRNE R5, [R6, #0x30]
	   BNE LOOP
	   
	   // if count is 0 turn off selected hex
	   MOV R8, #0b11111111111111111111111100000000
	   CMP R1,#1
	   ROREQ R8, #24
